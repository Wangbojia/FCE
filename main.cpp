...
vector<float> Ker_alg(int N, int K)  //好像不用全部return，记录下N，K就行,执行全部之后用NK求 RT，FT，CORE
...

{
	...
	while (stack.size() != 0) {
		...
		printv(schedstatus);
		//vector<int> prec = pred[cur];  //////////不应该是cur
	...
	printv(FTl);
	printv(FTwr);
	float finish_time = max(FTl[9], FTwr[9]);  ////////////////////////////////FTl 和 FTwr都没被更新
